...
## AG.0:1 - История изменений

| Дата | Версия | Изменения |
|------|--------|-----------|
| 2026-04-08 | v0.2 | В `AG.3:3` добавлены правила оформления сносок, включая многострочные сноски; уточнено, что сноски уникальны в пределах файла и располагаются в конце файла; в `AG.3` добавлены правила для внешних ссылок; добавлен журнал изменений документа; в `AG.2:8` для чеклистов добавлен алиас `Check` в табельных номерах |
| 2026-02-15 | v0.1 | Первая опубликованная версия гайда |
...
Мастер-данные всех руководств хранятся в репозитории https://github.com/aisystant/guides/
...
4. Автоматически запускается проверка формата (TODO: пока не реализовано)
...
7. Руководство публикуется на все ресурсы МИМ (TODO: пока не реализовано).
...
| **Чеклист**                  | `Checklist`, `Tasks`, `Check`       | `R5.1:Check1`            | См. `AG.5:1`                               |
...
Используйте стандартный Markdown-синтаксис сносок, совместимый с `markdown-it-footnote`.

**Ссылка в тексте:**

```markdown
Системное мышление[^1] помогает точнее обсуждать рабочие ситуации.
```
...
- Ссылка на сноску в тексте оформляется как `[^N]`, где `N` — уникальный номер сноски в пределах файла
- Определение сноски оформляется как `[^N]: текст сноски`
- Все определения сносок располагаются **в конце файла**
- В файле должны находиться **только те определения сносок, на которые есть ссылки** в тексте этого файла
- Нумерация сносок должна быть уникальна в пределах файла; пропуски в нумерации допустимы
- Форматы Word-экспорта вида `[\[1\]](#_ftn1)` и `[\[1\]](#_ftnref1)` не допускаются

**Одна пустая строка перед блоком сносок**

Это означает следующее: после последнего абзаца основного текста файла должна идти ровно одна пустая строка, и сразу после неё — первая сноска. Не нужно вставлять перед сносками горизонтальную линию (`---`), дополнительные заголовки или несколько пустых строк.
...
[^1]: Перед сносками две пустые строки.
```
...
[^1]: Перед сносками вставлена горизонтальная линия.
```

**Многострочные сноски**

Если текст сноски занимает несколько строк, все строки продолжения должны иметь отступ в 4 пробела.

```markdown
Системное мышление[^1] помогает точнее обсуждать рабочие ситуации.

[^1]: Первая строка сноски.
    Продолжение той же сноски на следующей строке.
    Ещё одно продолжение с тем же отступом.
```

**Пример файла с несколькими сносками:**

```markdown
# R5.1 - О мышлении

## R5.1:1 - Что такое системное мышление

Системное мышление[^1] помогает обсуждать сложные объекты.
Это согласуется и с современными инженерными стандартами[^2].

## R5.1:End

[^1]: Левенчук А.И. «Системное мышление», 2024.

[^2]: ISO/IEC/IEEE 15288:2023.
...
## AG.3:10 - Внешние ссылки

Для внешних ссылок используйте стандартный Markdown-формат:

```markdown
[Текст ссылки](https://example.com)
```

**Допустимо:** указывать просто URL без Markdown-обёртки, если ссылка сама по себе является содержательной и не требует отдельного текста.
...
```markdown
Актуальная версия FPF доступна в [репозитории FPF](https://github.com/ailev/FPF).

Исходники доступны по адресу https://github.com/aisystant/guides
```
